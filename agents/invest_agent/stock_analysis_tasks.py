...
class StockAnalysisTasks():
  def research(self, agent, company):
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output="""Summary of recent news articles, press
        releases, and market analyses related to the stock and
        its industry.""",
  ...
  def financial_analysis(self, agent, company): 
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output="""Analysis of the stock's financial
        health and market performance.""",
      #async_execution=True
    )
  ...
  def filings_analysis(self, agent, company):
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output="""Analysis of the latest 10-Q and 10-K filings from EDGAR for
        the stock in question.""",
      #async_execution=True
    )

  def strategy_analysis(self, agent, company):
    return Task(description=dedent(f"""
        You use all tools to check if all metrics from 
        the ticker respects the metrics developed by Benjamin Graham.

        Your final answer must be an expanded report that now
        also highlights significant findings from these metrics,
        including any red flags or positive indicators for
        your customer the methodology of Benjamin Graham.
        {self.__tip_section()}
  
        Selected company by the customer: {company}
      """),
      agent=agent,
      expected_output="""Analysis of metrics from Benjamin Graham
        the stock in question.""",
      #async_execution=True
    )
  ...
  def recommend(self, agent, company):
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output="""Synthesis of provided by the
        Financial Analyst and the Research Analyst."""
    )
